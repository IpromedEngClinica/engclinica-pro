-- ============================================================
-- EngClinica Pro
-- Migration: 044_convites_usuario_link_unico.sql
-- Objetivo:
-- - Remover limite de tecnicos por cliente
-- - Ajustar convites para exigir cliente apenas de solicitantes
-- - Criar funcoes de validacao e aceite de convite por token unico
-- ============================================================

drop index if exists public.idx_empresas_limite_tecnicos;

alter table public.empresas
drop constraint if exists empresas_limite_tecnicos_check;

alter table public.empresas
drop column if exists limite_tecnicos;

alter table public.usuario_convites
drop constraint if exists usuario_convites_empresa_check;

alter table public.usuario_convites
add constraint usuario_convites_empresa_check check (
...
set
  descricao = 'Editar matriz de permissoes e convites de acesso.',
  updated_at = now()
where chave = 'usuarios.gerenciar';
...
-- Fim da migration 044_convites_usuario_link_unico.sql
