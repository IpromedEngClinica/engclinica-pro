...
export const buildCalibracaoCertificadoHtml = (
  ...
  const notaConformidade = execucao.criterio_conformidade_aplicado
    ? `A declaracao de conformidade foi emitida conforme a regra de decisao registrada nesta calibracao: ${execucao.regra_decisao}.`
  ...
  <body><main class="document">
    <header><img src="${logoSrc}" alt="ACI"><div><h1>Certificado de Calibracao</h1><div class="meta"><strong>${numero}</strong><br>Emissao: ${date(execucao.data_emissao)}</div></div></header>
    ...
    <h2>3. Condicoes Ambientais</h2><section class="grid">
      ${field("Local", execucao.local_calibracao)}${field("Temperatura", `${decimal(execucao.temperatura_ambiente)} ${execucao.unidade_temperatura || ""}`)}
      ${field("Umidade relativa", `${decimal(execucao.umidade_relativa)} ${execucao.unidade_umidade || ""}`)}
      ${field("Pressao atmosferica", execucao.pressao_atmosferica == null ? "-" : `${decimal(execucao.pressao_atmosferica)} ${execucao.unidade_pressao || ""}`)}
    </section>
    <h2>4. Padroes Utilizados</h2><table><thead><tr><th>Nome</th><th>Certificado</th><th>Validade</th><th>Identificacao</th><th>Orgao calibrador</th></tr></thead>
      <tbody>${padroes.map((p) => `<tr><td>${esc(p.nome)}</td><td>${esc(p.certificado)}</td><td>${date(p.validade)}</td><td>${esc(p.identificacao)}</td><td>${esc(p.laboratorio)}</td></tr>`).join("")}</tbody></table>
    ...
    <h2>7. Resultados</h2>
    ${(execucao.tabelas || []).map((tabela) => `<h3>${esc(tabela.nome_snapshot)}</h3><div>Valor de uma divisao: ${decimal(tabela.resolucao_equipamento_snapshot)} ${esc(tabela.unidade_snapshot)}</div>
      ...
      <tbody>${(tabela.pontos || []).map((ponto) => `<tr><td>${decimal(ponto.valor_nominal)}</td><td>${decimal(ponto.media_valores_medidos)}</td><td>${decimal(ponto.tendencia_corrigida)}</td><td>${decimal(ponto.incerteza_expandida)}</td><td>${decimal(ponto.fator_abrangencia_k)}</td>${execucao.criterio_conformidade_aplicado ? `<td>${esc(ponto.resultado_conformidade)}</td>` : ""}</tr>`).join("")}</tbody></table>`).join("")}
    <h2>Resumo da Calibracao</h2><section class="grid">${field("Local", execucao.local_calibracao)}${field("Data da calibracao", date(execucao.data_calibracao))}${field("Emitido em", date(execucao.data_emissao))}${field("Valido ate", date(execucao.data_validade))}${field("Numero", numero)}${execucao.criterio_conformidade_aplicado ? field("Resultado geral", execucao.resultado_geral) : ""}</section>
    <div class="sign"><div class="line">${esc(execucao.tecnico_executor_nome)}<br>${esc(execucao.tecnico_executor_registro)}<br>Tecnico Executor</div><div class="line">${esc(execucao.responsavel_tecnico_nome)}<br>${esc(execucao.responsavel_tecnico_registro)}<br>Responsavel Tecnico / Signatario Autorizado</div></div>
