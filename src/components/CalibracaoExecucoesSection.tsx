...
const CalibracaoExecucoesSection = () => {
  ...
  const getters = useMemo<Record<string, (item: CalibracaoExecucao) => unknown>>(() => ({
    numero_certificado: (item) => item.numero_certificado, data_calibracao: (item) => item.data_calibracao, cliente: (item) => item.empresa?.nome || item.empresa?.nome_fantasia,
  ...
  return <div className="space-y-4">
    ...
    <div className="rounded-xl border bg-card">
      ...
      {filtrosAbertos && <div className="grid gap-3 border-t p-4 md:grid-cols-2 xl:grid-cols-3">
        <Sel value={empresa} onChange={setEmpresa} label="Empresa" options={unique(execucoes.map((item) => [item.empresa_id, item.empresa?.nome || item.empresa?.nome_fantasia || "-"]))} />
