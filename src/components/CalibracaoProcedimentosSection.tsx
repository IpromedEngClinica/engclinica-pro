...
const CalibracaoProcedimentosSection = () => {
  ...
  const filtered = useMemo(() => procedimentos.filter((item) => {
    ...
    const geral = !termo || [item.nome, item.tipo_equipamento?.nome].some((value) => value?.toLowerCase().includes(termo));
    return geral && (tipoFiltro === ALL || item.tipo_equipamento?.nome === tipoFiltro) && (statusFiltro === ALL || (statusFiltro === "ativo") === item.ativo) && (!nomeFiltro.trim() || item.nome.toLowerCase().includes(nomeFiltro.trim().toLowerCase()));
  ...
  const getters = useMemo<Record<string, (item: CalibracaoProcedimento) => unknown>>(() => ({
    nome: (item) => item.nome,
    tipo: (item) => item.tipo_equipamento?.nome,
